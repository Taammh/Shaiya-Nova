...
import { Category, Faction, CLASSES_BY_FACTION, Gender, StaffApplication, DropMap, MobEntry, DropEntry, MapPoint, ItemRarity, GameItem, DropSubItem } from '../types';
...
const AdminPanel: React.FC = () => {
  ...
  const [uploadTarget, setUploadTarget] = useState<{ mobIdx: number, dropIdx?: number, subIdx?: number } | null>(null);
  ...
  const initialDrop: Partial<DropMap> = {
    name: '', category: 'Mapa', faction: Faction.NEUTRAL, image: '', description: '', mobs: []
  };
  ...
  const generateMasterLink = () => {
    ...
    alert("¡LINK MAESTRO CORTO GENERADO!");
  };

  const handleMassSync = async () => {
    if (!confirm("¿Deseas subir todo a Supabase?")) return;
    setIsSaving(true);
    try {
      const result = await massSyncLocalToCloud();
      alert(`Sincronización Exitosa: ${result.itemsCount} ítems, ${result.dropsCount} drops.`);
      loadData();
    ...
    try {
      ...
      if (type === 'item') setNewItem(prev => ({ ...prev, image: url }));
        ...
        setNewDrop(prev => {
          ...
          if (uploadTarget.subIdx !== undefined) {
            const contents = [...(drops[uploadTarget.dropIdx].chestContents || [])];
            contents[uploadTarget.subIdx] = { ...contents[uploadTarget.subIdx], image: url };
            drops[uploadTarget.dropIdx] = { ...drops[uploadTarget.dropIdx], chestContents: contents };
          } else {
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-10 pb-20 px-4">
          ...
          <div className="glass-panel p-8 rounded-[2.5rem] border-[#d4af37]/20 shadow-2xl">
            ...
            {newItem.category === Category.COSTUME && (
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6 animate-fade-in">
                <select className="w-full bg-black/60 border border-[#d4af37]/30 p-4 rounded-xl text-white outline-none" value={newItem.faction} onChange={e => setNewItem({...newItem, faction: e.target.value as Faction})}>
                  <option value={Faction.LIGHT}>Fila Luz</option><option value={Faction.FURY}>Fila Furia</option>
                </select>
                <select className="w-full bg-black/60 border border-[#d4af37]/30 p-4 rounded-xl text-white outline-none" value={newItem.item_class} onChange={e => setNewItem({...newItem, item_class: e.target.value})}>
                  <option value="All">Todas las Clases</option>
                  {(CLASSES_BY_FACTION[newItem.faction || Faction.LIGHT] || []).map(c => <option key={c} value={c}>{c}</option>)}
                </select>
                <select className="w-full bg-black/60 border border-[#d4af37]/30 p-4 rounded-xl text-white outline-none" value={newItem.gender} onChange={e => setNewItem({...newItem, gender: e.target.value as Gender})}>
                  <option value={Gender.BOTH}>Ambos Sexos</option><option value={Gender.MALE}>Hombres</option><option value={Gender.FEMALE}>Mujeres</option>
                </select>
              </div>
            )}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
              <div className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  ...
                  <select className="bg-black/60 border border-[#d4af37]/30 p-4 rounded-xl text-white outline-none" value={newDrop.faction} onChange={e => setNewDrop({...newDrop, faction: e.target.value as Faction})}>
                    <option value={Faction.NEUTRAL}>Neutral</option><option value={Faction.LIGHT}>Luz</option><option value={Faction.FURY}>Furia</option>
                  </select>
                </div>
                <div className="flex gap-2">
                  ...
                  {newDrop.mobs?.map((mob, mIdx) => (
                    <div key={mob.id} className={`p-5 rounded-3xl border transition-all ${activeMobIdx === mIdx ? 'border-[#d4af37] bg-[#d4af37]/10' : 'border-white/5 bg-black/40'}`} onClick={() => setActiveMobIdx(mIdx)}>
                      <div className="flex items-center gap-4 mb-4">
                        ...
                        <input className="bg-transparent border-b border-white/10 text-white font-bold outline-none flex-grow" value={mob.name} onClick={e => e.stopPropagation()} onChange={e => { const ms = [...(newDrop.mobs || [])]; ms[mIdx].name = e.target.value; setNewDrop({...newDrop, mobs: ms}); }} />
                      ...
                      {activeMobIdx === mIdx && (
                        <div className="space-y-4 pt-3 border-t border-white/5">
                          {mob.drops?.map((drop, dIdx) => (
                            <div key={dIdx} className="bg-black/60 p-3 rounded-2xl border border-white/5 space-y-3">
                               <div className="flex items-center gap-2">
                                 <div className="relative group shrink-0">
                                   <img src={drop.itemImage || "https://api.dicebear.com/7.x/pixel-art/svg?seed=fallback"} className="w-10 h-10 rounded-lg bg-black" />
                                 ...
                                 <input className="flex-grow bg-transparent text-white text-[10px] outline-none" value={drop.itemName} onClick={e => e.stopPropagation()} onChange={e => { const ms = [...(newDrop.mobs || [])]; ms[mIdx].drops[dIdx].itemName = e.target.value; setNewDrop({...newDrop, mobs: ms}); }} />
                                 <button 
                                   onClick={(e) => { e.stopPropagation(); const ms = [...(newDrop.mobs || [])]; ms[mIdx].drops[dIdx].isChest = !ms[mIdx].drops[dIdx].isChest; if(ms[mIdx].drops[dIdx].isChest && !ms[mIdx].drops[dIdx].chestContents) ms[mIdx].drops[dIdx].chestContents = []; setNewDrop({...newDrop, mobs: ms}); }}
                                   className={`p-2 rounded-lg text-xs ${drop.isChest ? 'bg-[#d4af37] text-black' : 'bg-white/5 text-white opacity-40 hover:opacity-100'}`}
                                   title="Configurar como Cofre"
                                 >📦</button>
                               ...
                               {drop.isChest && (
                                 <div className="pl-6 border-l border-[#d4af37]/30 space-y-3 animate-fade-in">
                                   <div className="flex justify-between items-center mb-2">
                                     <span className="text-[#d4af37] text-[8px] font-black uppercase tracking-widest">Contenido del Cofre</span>
                                     <button onClick={() => { const ms = [...(newDrop.mobs || [])]; const sub: DropSubItem = { name: 'Sub-Item', image: '', rate: '10%', rarity: 'Common' }; ms[mIdx].drops[dIdx].chestContents = [...(ms[mIdx].drops[dIdx].chestContents || []), sub]; setNewDrop({...newDrop, mobs: ms}); }} className="text-[7px] bg-green-600/40 text-white px-2 py-1 rounded-md uppercase font-black">+ AGREGAR</button>
                                   </div>
                                   {drop.chestContents?.map((sub, sIdx) => (
                                      <div key={sIdx} className="flex items-center gap-2 bg-black/40 p-2 rounded-xl border border-white/5">
                                        <div className="relative group shrink-0">
                                          <img src={sub.image || "https://api.dicebear.com/7.x/pixel-art/svg?seed=fallback"} className="w-8 h-8 rounded-lg bg-black" />
                                          <button onClick={(e) => { e.stopPropagation(); setUploadTarget({ mobIdx: mIdx, dropIdx: dIdx, subIdx: sIdx }); subItemFileRef.current?.click(); }} className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 flex items-center justify-center text-[5px] font-black text-white">UP</button>
                                        </div>
                                        <input className="flex-grow bg-transparent text-white text-[9px] outline-none" value={sub.name} onChange={e => { const ms = [...(newDrop.mobs || [])]; ms[mIdx].drops[dIdx].chestContents![sIdx].name = e.target.value; setNewDrop({...newDrop, mobs: ms}); }} />
                                        <select className="bg-black/40 text-[#d4af37] text-[8px] font-black rounded border border-white/5 px-1 outline-none" value={sub.rarity} onChange={e => { const ms = [...(newDrop.mobs || [])]; ms[mIdx].drops[dIdx].chestContents![sIdx].rarity = e.target.value as ItemRarity; setNewDrop({...newDrop, mobs: ms}); }}>
                                           {['Common', 'Noble', 'Atroz', 'Legendary', 'Diosa', 'Special', 'Unique'].map(r => <option key={r} value={r}>{r}</option>)}
                                        </select>
                                        <input className="w-8 bg-transparent text-blue-400 text-[8px] outline-none text-center" value={sub.rate} onChange={e => { const ms = [...(newDrop.mobs || [])]; ms[mIdx].drops[dIdx].chestContents![sIdx].rate = e.target.value; setNewDrop({...newDrop, mobs: ms}); }} />
                                        <button onClick={() => { const ms = [...(newDrop.mobs || [])]; ms[mIdx].drops[dIdx].chestContents!.splice(sIdx, 1); setNewDrop({...newDrop, mobs: ms}); }} className="text-red-900 text-[8px]">×</button>
                                      </div>
                                   ))}
                                 </div>
                               )}
      ...
      <input type="file" ref={subItemFileRef} className="hidden" accept="image/*" onChange={(e) => handleFileUpload(e, 'drop')} />
