...
const BugReportForm: React.FC = () => {
  ...
  const handleFinishLogin = () => {
    if (!manualDiscordTag.includes('#') && manualDiscordTag.length < 3) {
      alert("Introduce un Discord Tag válido (Ejemplo: Shaiya#0001)");
      return;
    }
    ...
    const userData = { 
      ...
      avatar: `https://api.dicebear.com/7.x/pixel-art/svg?seed=${manualDiscordTag}` 
    };
  ...
  if (!isLoggedIn) {
    return (
      <div className="max-w-xl mx-auto glass-panel p-16 rounded-[3rem] text-center space-y-10 animate-fade-in shadow-[0_0_50px_rgba(88,101,242,0.15)] relative overflow-hidden border-[#5865F2]/30">
        <div className="absolute top-0 left-0 w-full h-2 bg-gradient-to-r from-transparent via-[#5865F2] to-transparent"></div>
        
        <div className="w-32 h-32 bg-[#5865F2] rounded-[2.5rem] mx-auto flex items-center justify-center shadow-2xl shadow-[#5865F2]/40 animate-glow">
        ...
        <div>
          <h2 className="text-4xl font-shaiya text-white mb-4 uppercase tracking-[8px]">Acceso Denegado</h2>
          <p className="text-gray-400 text-xs uppercase tracking-[4px] leading-relaxed">Debes vincular tu cuenta de Discord para abrir un ticket de soporte en NOVA.</p>
        </div>

        {!showLoginModal ? (
          <button 
            onClick={() => setShowLoginModal(true)}
            className="w-full bg-[#5865F2] hover:bg-white hover:text-[#5865F2] text-white font-black py-6 rounded-[1.5rem] transition-all uppercase tracking-widest shadow-xl active:scale-95"
          >
          ...
          <div className="space-y-4 animate-fade-in">
            <input 
              type="text"
              placeholder="Discord Tag (Ej: Shaiya#0001)"
              className="w-full bg-black/60 border-2 border-[#5865F2]/50 p-5 rounded-2xl text-white text-center outline-none focus:border-[#5865F2] transition-all"
            ...
            <button 
              ...
              className="w-full bg-white text-black font-black py-5 rounded-2xl uppercase tracking-widest hover:bg-[#5865F2] hover:text-white transition-all"
            >
              Completar Vinculación
            </button>
            <button onClick={() => setShowLoginModal(false)} className="text-gray-500 text-[10px] uppercase font-bold tracking-widest">Cancelar</button>
          </div>
        )}
      </div>
    );
    ...
    <div className="max-w-3xl mx-auto glass-panel p-10 rounded-[3rem] border-[#d4af37]/40 shadow-2xl relative animate-fade-in">
      <div className="flex items-center justify-between mb-10 pb-6 border-b border-white/10">
        <div className="flex items-center gap-5">
          ...
          <div>
            <p className="text-[#d4af37] text-[10px] font-black uppercase tracking-[4px]">Vínculo Establecido</p>
            <p className="text-white text-2xl font-shaiya">{discordUser?.name}</p>
          </div>
        </div>
        <button onClick={handleLogout} className="text-gray-500 text-xs hover:text-red-400 font-bold uppercase tracking-tighter transition-colors">Desvincular</button>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        {status === 'success' && <p className="text-green-400 text-center font-black animate-bounce mt-4 tracking-widest">¡El mensaje ha sido entregado!</p>}
        {status === 'error' && <p className="text-red-400 text-center font-black mt-4 tracking-widest uppercase text-xs">Fallo en la conexión. Webhook no válido.</p>}
      </form>
    </div>
  );
};
