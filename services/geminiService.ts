...
export const getLoreForItem = async (itemName: string, category: string): Promise<string> => {
  try {
    // Fixed: Use named parameter and obtain API key directly from process.env.API_KEY as per guidelines
    ...
    // Fixed: Accessed .text property directly (it's not a method)
